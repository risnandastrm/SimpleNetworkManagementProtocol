...
#define SNMP_DEFAULT_PORT	161 //161
...
public:
	...
	void freeTrapPdu(SNMP_TRAP_PDU *pdu);
	
	void Trap(char Message[], byte RemIP[4], byte AgntIP[4], uint32_t Tiempo, SNMP_TRAP_PDU *pduu);
	// Helper functions

private:
	byte _packet[SNMP_MAX_PACKET_LEN];
	byte _packetTrap[SNMP_MAX_PACKET_LEN];
	
	uint16_t 	totalOID,	//add
				totalVAL,
				total;

	uint16_t _packetSize;
	uint16_t _packetSizeTrap;
	
	uint16_t _packetPos;
	uint16_t _packetPosTrap;
